...
/**************************************************************************************************************/
/* SLQ _1 - OK */
drop table if exists #TAXES_INFO_TA
go
...
/******************************************************************************************************************/
/* SLQ _2 - OK */
drop table if exists #TAXES_INFO_COMMISSION_TA
go
SELECT	DISTINCT REG.CODE			AS TAX_CODE
...
INNER JOIN [TESTSERVER51\SQL2016].[BPB_Next_VCSBank_TestAutomation_Online].DBO.DT008 AS [CURR] WITH(NOLOCK)
	ON COMM.CURRENCY = CURR.CODE
WHERE	REG.TA_TABLE = N'PREV_COMMON_TA'
	AND REG.[TYPE]	 = N'Tax'
ORDER BY REG.CODE, CURR.INI, COMM.COMMISSION_CODE, COMM.UP_TO_SUM
GO
